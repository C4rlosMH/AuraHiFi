...
// 🚀 NUESTRO ATRAPA-ERRORES BLINDADO (Modo Fallo-Rápido 3s)
export const fetchFromNavidrome = async (url: string, timeoutMs: number = 3000) => {
    ...
    try {
        ...
        clearTimeout(timeoutId);
        // Si el internet está apagado o pasaron 3 segundos, abortamos controladamente
        if (error.name === 'AbortError' || error.message?.includes('Network request failed') || error.message?.includes('Failed to fetch')) {
            console.warn(`⏳ OFFLINE/TIMEOUT: El servidor no respondió a ${url.split('?')[0]}`);
